...
if (isset($_POST['signup_btn'])) {
	...
	if (count($errors) === 0) {
		...
		$query = "INSERT INTO users SET fname=?,lname=?,username=?, email=?,address=?, password=?, profile_picture=?, created_at=?";
		$stmt = $conn->prepare($query);
		$stmt->bind_param('ssssssss',$fname,$lname, $username, $email,$address, $password, $profile_picture, $created_at);
		$result = $stmt->execute();
		if ($result) {
